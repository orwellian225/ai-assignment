...
def calculate_entropy(states: set[str], turn: chess.Color):
    if len(states) == 0:
        return None

    probabilites = [None for _ in chess.SQUARES]
    ...
    for state in states:
        ...
        for square in chess.SQUARES:
            ...
            if current_piece not in probabilites[square].keys():
                probabilites[square][current_piece] = 1
            else:
                probabilites[square][current_piece] += 1
    ...
    for square in chess.SQUARES:
        for pieces in probabilites[square].keys():
            entropies[square] -= (probabilites[square][pieces] / num_states) * m.log(probabilites[square][pieces] / num_states, 2)
...
calculate_entropy(set(), chess.WHITE)
...
class NonPrunedEntropic(rc.Player):
    def __init__(self):
        self.colour = False
        self.my_board = None
        self.current_move = 0
        self.states = set()
        self.logger = logging.getLogger('sensing.entropic')
        logging.basicConfig(filename="improved-2.log", encoding="utf-8", level=logging.DEBUG)
    ...
    def handle_opponent_move_result(self, captured_my_piece: bool, capture_square: int | None):
        self.current_move += 1
        self.logger.debug(f'Move {self.current_move}')
        self.logger.debug('--------------------------------')
        self.logger.debug(f'{self.my_colour} in {self.my_board.board_fen()}')
        self.logger.debug(f'{self.opp_colour} in {len(self.states)} possible states')
        ...
        self.logger.debug(f'Opp move result:\tremoved {num_removed_states}')
    ...
    def choose_sense(self, sense_actions: list[chess.Square], move_actions: list[chess.Move], seconds_left: float) -> chess.Square | None:
        ...
        entropy = calculate_entropy(self.states, self.colour)
        ...
        self.logger.debug(f'Sense choice:\t{chess.square_name(selected_sense_square)} with entropy {np.reshape(entropy, (6*6))[selected_indices]} | max entropy {np.max(entropy)}')
    ...
    def handle_sense_result(self, sense_result: rc.List[rc.Tuple[chess.Square | chess.Piece | None]]):
        ...
        self.logger.debug(f'Sense result:\t\tremoved {len(removed_states)} of {before_state_size} | {len(removed_states) / before_state_size if before_state_size != 0 else 1e6 * 100:.2f}%')

    def choose_move(self, move_actions: list[chess.Move], seconds_left: float) -> chess.Move | None:
        search_states = set()

        if len(self.states) > 10_000:
            search_states = random.sample(list(self.states), 10_000)
        else:
            search_states = self.states

        self.logger.debug(f'Stockfish search:\t{len(search_states)} states at {10 / len(search_states) if len(search_states) != 0 else 1e6} seconds per state')
        board = chess.Board()
        selected_moves = {}
        for state in search_states:
            ...
            try:
                ...
                self.logger.warning(f"Engine died: state {state}")
            except chess.engine.EngineError:
                self.logger.warning(f'Engine bad state {state}')
        ...
        if len(selected_moves) != 0:
            ...
            self.logger.debug(f"Nonexistant moves:\t{nonexisting_moves}")
    ...
    def handle_move_result(self, requested_move: chess.Move | None, taken_move: chess.Move | None, captured_opponent_piece: chess.Color, capture_square: chess.Square | None):
        ...
        self.logger.debug(f'Move choice:\t\t{requested_move.uci() if requested_move else "0000"}')
        self.logger.debug(f'Move taken:\t\t{taken_move.uci() if taken_move else "0000"}')
        ...
        if taken_move:
            ...
            for removed_state in removed_states:
                try:
                    ...
                    self.logger.warning(f"Trying to remove non-existant state {removed_state}")

        self.logger.debug(f'My move result:\t\tremoved {num_invalid_move_for_state_removed + num_invalid_move_taken_removed} of {before_state_size} | {(num_invalid_move_for_state_removed + num_invalid_move_taken_removed) / before_state_size if before_state_size != 0 else 1e6 * 100:.2f}%')
